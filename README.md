...
미연결 파일의 Sub 열기를 차단하고 이전 파일·탭을 유지합니다. Card는 목록으로 고정하며 파일 유형을 복수 선택합니다. 파일 목록과 가상 폴더를 기존 Canvas에 위치 지정 → 미리보기 → 겹침 확인 → 확정으로 삽입합니다. [0.1.8 Release](https://github.com/tlatndms2-droid/md-palette/releases/tag/0.1.8) · [검증 결과](CANVAS_REVISION_VALIDATION.md). BRAT 사용자 확인 대기입니다.

### 기존 Sub 지정 기능

Sub 그룹을 하나만 유지하고 탭 우클릭으로 지정·해제합니다. 미연결 파일은 연결 승인 후 지정하며, Ctrl+Shift+더블클릭은 새 일반 그룹으로 엽니다. Sub 아래 분할도 허용합니다. [0.1.7 Release](https://github.com/tlatndms2-droid/md-palette/releases/tag/0.1.7) · [검증 결과](SUB_RULES_VALIDATION.md). BRAT 사용자 확인 대기입니다.
...
Sub가 없으면 Main 오른쪽에 만들며, 이후 사용자 배치를 강제로 바꾸지 않습니다. Sub 아래에 다른 일반 그룹을 분할해 놓아도 기존 Sub 지정을 유지합니다. 0.1.7부터 과거 전체 높이 고정과 여러 Sub 생성 규칙을 적용하지 않습니다.
...
3. 기본 더블클릭은 유일한 Sub의 현재 탭을 교체합니다. Ctrl+더블클릭은 같은 Sub에 새 탭, Ctrl+Shift+더블클릭은 **새 일반 그룹**을 추가합니다. Sub에 같은 파일이 열려 있으면 기존 탭을 보여줍니다. 새 일반 그룹 추가는 같은 파일도 허용합니다.
...
Main과 다른 그룹의 파일 탭 제목 우클릭 → `서브 스페이스로 지정 / 서브 스페이스 지정 해제`. 명령 `서브 스페이스 지정/해제`도 제공합니다. 미연결 파일은 `Main에 연결하시겠습니까?`에서 `연결하고 Sub로 지정 / 취소`를 선택합니다. 이미지는 물론 Canvas도 연결할 수 있으며, 취소하면 연결과 역할이 바뀌지 않습니다. 다른 그룹을 지정하면 이전 Sub는 파일과 탭을 남기고 일반 그룹으로 돌아갑니다.
...
재시작하면 Obsidian이 복원한 기존 그룹에 Main과 단일 Sub 역할을 다시 연결합니다. 과거 여러 Sub 저장값에서는 마지막 사용 그룹 하나만 Sub로 남기고 나머지는 일반 그룹으로 보존합니다. Card·Folder·Connections 파일 목록의 한 번 클릭은 선택이며 Ctrl·Shift 선택은 유지합니다. 열기 조합은 더블클릭에 적용합니다. 없는 탭이나 이전 파일을 강제로 다시 열지 않습니다.
...
- MD·Canvas·PDF·이미지·영상·기타를 여러 개 함께 선택합니다. 전체는 모든 유형과 연동하며 라벨 필터와 함께 적용됩니다. 라벨의 `All`은 개별 라벨 선택을 해제합니다. 필터를 접어도 선택 조건은 유지됩니다.
- `라벨 관리…`에서 라벨별 전체 Vault 사용 파일 수와 파일 이름·폴더를 확인합니다. `라벨 삭제 → 라벨만 삭제`는 해당 라벨·파일에 붙은 표시·필터만 제거하며 실제 파일은 보존합니다.
- Card의 목록, Folder의 파일 카드·Tree 파일 행, Connections의 파일 목록에 Ctrl을 누르고 마우스를 올리면 Obsidian 기본 페이지 미리보기가 나타납니다. 라벨 관리의 파일 목록도 지원합니다. 가상 폴더·버튼·메타데이터 항목은 대상이 아닙니다. 코어 플러그인 `페이지 미리보기`가 켜져 있어야 합니다.
...
- Card는 목록으로 고정하고 보기 형식 선택과 Ctrl+휠 보기 전환을 제거했습니다. 글자 크기 3종은 유지합니다. Folder의 보기 형식은 변경하지 않습니다.
- 파일 제목은 Card와 Folder의 파일 카드 상단에 더 크게 표시합니다. Tree 목록과 가상 폴더 이름 크기는 유지합니다.
- Card 목록은 파일 아이콘·이름·라벨을 표시합니다. Folder의 기존 썸네일 보기는 유지합니다.

라벨·필터·보기 형식·글자 크기·카드 순서는 재시작 후 복원됩니다. 라벨과 순서 변경은 Markdown 내용이나 실제 파일 위치를 바꾸지 않습니다.

## Canvas에 파일과 폴더 삽입

1. 기존 Canvas를 열고 Card에서 파일을 고릅니다. Ctrl+클릭으로 여러 개, Shift+클릭으로 범위를 선택합니다.
2. 파일 우클릭 → `현재 Canvas에 삽입…` 또는 Canvas로 드래그합니다. 여러 Canvas 중 대상이 불분명하면 열린 목록에서 고릅니다.
3. Canvas 위치를 클릭해 점선 미리보기를 확인합니다. 겹치면 확정할 수 없으며 다른 위치를 클릭해 옮깁니다.
4. `이 위치에 삽입 확정`을 누르면 원본 Vault 파일을 참조하는 노드를 만듭니다. `취소`/Esc는 변경하지 않습니다.
5. Folder의 폴더 우클릭 → `폴더를 현재 Canvas에 삽입…`, 전체 루트 우클릭 → `전체 가상 폴더를 현재 Canvas에 삽입…`. 폴더 제목·파일 노드와 부모·자식 연결선을 함께 배치합니다. 현재 화면의 파일 유형 필터와 관계없이 선택 폴더의 모든 자손을 포함합니다.
6. 명령 팔레트의 `MD Palette: 마지막 Canvas 삽입 되돌리기`로 현재 실행 중 마지막 삽입을 취소합니다. 삽입한 노드에 새 연결선을 추가했다면 자동 삭제하지 않고 알립니다.

원본 파일 이동·복사와 새 Canvas 자동 생성은 없습니다. 폴더 삽입은 해당 시점 배치이며 이후 Palette 편집으로 자동 재배치하지 않습니다. 지원하지 않는 파일 및 대상 Canvas 자체를 포함한 삽입은 사전에 알리고 취소합니다.
...
- 읽기 모드·일반 문서·이미지·PDF·영상 등에는 삽입하지 않습니다. Card 목록의 파일은 아래 Canvas 삽입 흐름으로 단일·복수 삽입합니다. 메뉴가 열린 사이 원문·대상·Space가 변경되면 다시 끌어 놓도록 안내합니다.
