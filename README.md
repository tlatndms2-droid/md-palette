...
사용 순서: `연결 깊이 선택 → 파일 옆 화살표로 하위 링크 펼치기 → 더블클릭으로 Sub 열기`. Metadata는 `Metadata View → 정보를 볼 파일 → 파일 선택`으로 확인합니다.
...
3. 직접 드래그는 놓는 즉시 배치합니다. 기존 카드 본문 위에도 놓을 수 있습니다.
4. 메뉴 삽입은 점선 미리보기가 마우스를 따라옵니다. 원하는 위치를 클릭하면 배치하며 별도 확정 버튼은 없습니다. 다중 카드 묶음이 겹치면 빈 위치로 옮깁니다. 단일 카드는 겹침을 허용합니다. 배치 전 `취소`/Esc는 변경하지 않습니다.
