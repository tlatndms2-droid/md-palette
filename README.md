...
- Links: Main 본문과 모든 속성의 링크를 중복 없이 표시합니다. 내부 파일은 같은 그룹의 Main 옆 탭에서 열며 기존 파일 탭을 재사용합니다. Main 기준은 유지됩니다.
- 웹 링크: 매번 옵시디언 웹뷰어 / 기본 브라우저를 선택합니다. 웹뷰어가 꺼져 있으면 `설정 → 코어 플러그인 → 웹 뷰어`에서 켜는 위치를 안내합니다.
