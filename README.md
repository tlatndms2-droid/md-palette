...
- `Link View → Card`의 파일 카드 한 개를 Main 문서의 **편집 모드 본문**에 놓습니다. 메뉴에서 `링크 삽입`, `임베드 삽입`, Markdown 파일의 `본문 Markdown 삽입`을 고릅니다. 본문 삽입은 속성을 제외하며 원본 파일은 남습니다.
...
- 읽기 모드·일반 문서·이미지·PDF·영상 등에는 삽입하지 않습니다. 파일 카드는 Main만 받습니다. 메뉴가 열린 사이 원문·대상·Space가 변경되면 다시 끌어 놓도록 안내합니다.
- 기존 카드 사이 드래그 정렬은 유지합니다. Folder·Connections, Tasks·각주·웹 링크에는 새 삽입 드래그를 추가하지 않습니다.
...
4. 표시 버전이 `0.0.12`인지 확인합니다. 기존 설치자는 BRAT 업데이트를 실행합니다.
