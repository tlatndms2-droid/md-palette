...
export type ReuseMode = 'link' | 'embed' | 'body' | 'text' | 'source' | 'footnote' | 'footnote-context' | 'address' | 'named-url';
...
export function reuseOptions(kind: ReuseKind, markdownFile: boolean, canvas: boolean): Array<{ mode: ReuseMode; title: string }> {
  if (kind === 'footnote') return [{ mode: 'text', title: '각주만' }, { mode: 'footnote-context', title: '본문만' }, { mode: 'footnote', title: '각주와 본문' }];
...
export function footnoteInsertion(original: string, offset: number, content: string, context = ''): { changes: Array<{ offset: number; text: string }>; value: string } {
  ...
  const id = `mdp-${n}`, marker = context.replace(/\r\n?/g, '\n') + `[^${id}]`;
