...
const c=await connect(),dir='.artifacts/stage2-drag',mode=process.argv[2];
...
try{
  ...
  if(mode==='prepare'){
    ...
    const baseline='.artifacts/stage2/validated-obsidian/plugins';
    assert.equal(JSON.parse(await readFile(`${baseline}/md-palette/manifest.json`,'utf8')).version,'0.0.3');
    ...
    assert.equal(state.version,'0.0.3');assert.ok(state.tracked.includes('tlatndms2-droid/md-palette'));console.log(state);
  }else{
    if(mode!=='verify'){
      assert.equal(await c.evaluate(`app.plugins.plugins['md-palette'].manifest.version`),'0.0.3');
    ...
    let live;for(let i=0;i<150;i++){live=await c.evaluate(`(()=>{const p=app.plugins.plugins['md-palette'];return {version:p?.manifest.version,enabled:app.plugins.enabledPlugins.has('md-palette'),spaces:p?.data.spaces,cards:p?.cards,unknown:p?.data.preservedFixture}})()`);if(live.version==='0.0.4'&&live.spaces?.main)break;await pause(200);}
    assert.equal(live.version,'0.0.4');assert.ok(live.enabled);
    const saved=JSON.parse(await readFile(`${dir}/data-before-brat.json`,'utf8'));
    assert.deepEqual(live.spaces,saved.spaces);assert.deepEqual(live.cards,saved.cards);assert.deepEqual(live.unknown,saved.preservedFixture);
    const assets=[];
    ...
    assert.equal(await c.evaluate(`document.querySelectorAll('.mdp-card').length`),411);
    await c.screenshot(`${dir}/brat-updated.png`);assert.deepEqual(c.errors,[]);
    await writeFile(`${dir}/brat-verification.json`,JSON.stringify({passed:true,from:'0.0.3',to:'0.0.4',cardsAndSpacesPreserved:true,unknownDataPreserved:true,assets,errors:c.errors},null,2));console.log('PASS BRAT 0.0.3 → 0.0.4, cards, spaces and installed assets');
  }
}finally{c.close();}
