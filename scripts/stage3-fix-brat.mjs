...
const c=await connect(),dir='.artifacts/stage3-fix',mode=process.argv[2];
...
try{
  ...
  if(mode==='prepare'){
    ...
    const baseline='.artifacts/stage3/validated-obsidian/plugins';
    assert.equal(JSON.parse(await readFile(`${baseline}/md-palette/manifest.json`,'utf8')).version,'0.0.5');
    ...
    assert.equal(state.version,'0.0.5');assert.ok(state.tracked.includes('tlatndms2-droid/md-palette'));console.log(state);
  }else{
    if(mode!=='verify'){
      assert.equal(await c.evaluate(`app.plugins.plugins['md-palette'].manifest.version`),'0.0.5');
    ...
    assert.equal(await c.evaluate(`document.querySelectorAll('.mdp-native-graph canvas').length`),1);
    await c.screenshot(`${dir}/brat-updated.png`);assert.deepEqual(c.errors,[]);
    await writeFile(`${dir}/brat-verification.json`,JSON.stringify({passed:true,from:'0.0.5',to:'0.0.6',cardsAndSpacesPreserved:true,unknownDataPreserved:true,assets,errors:c.errors},null,2));console.log('PASS BRAT 0.0.5 → 0.0.6, cards, spaces and installed assets');
  }
}finally{c.close();}
