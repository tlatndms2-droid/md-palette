...
test('both directions retain provenance, shared children appear below each parent, ancestry stops cycles',()=>{
 const i=new LinkIndex({main:{a:1,d:1},a:{b:1},b:{a:1,main:1},c:{a:1},d:{c:1},self:{self:1}});
 assert.deepEqual(i.children('a',['main','a']),[{path:'b',outgoing:true,incoming:true},{path:'c',outgoing:false,incoming:true}]);
...
test('depth traversal is shortest-path bounded and safe in cycles',()=>{
 ...
 assert.equal(i.reachable('main',3).has('c'),true);
...
test('large fanout remains linear rather than enumerating every possible path',()=>{
 ...
 const i=new LinkIndex(links);assert.equal(i.reachable('main',5).size,5002);assert.equal(i.children('shared',['main','n0','shared']).length,4999);
});
