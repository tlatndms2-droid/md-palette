...
test('reorder preserves hidden entries and survives read/save without discarding manual order',()=>{const s=fixture();moveItems(s,['f:hidden.md','f:one.md'],'a','d:b');assert.deepEqual(s.order,['d:a','f:one.md','f:hidden.md','d:b','f:two.md','d:c']);s.sort='size';s.descending=true;const loaded=readFolders(JSON.parse(JSON.stringify(s)));assert.deepEqual(loaded.order,s.order);assert.equal(loaded.sort,'size');});
test('corrupt folders recover cycles and missing parents without infinite traversal',()=>{const s=readFolders({folders:[null,{id:'a',name:'A',parent:'b'},{id:'b',name:'B',parent:'a'},{id:'c',name:'C',parent:'missing'}],positions:{x:'missing',y:'b'},split:Infinity,current:'missing'});assert.equal(s.current,'');assert.equal(s.split,.5);assert.equal(s.positions.x,undefined);assert.ok(ancestors(s,'b').length<=2);assert.equal(s.folders.find(f=>f.id==='c').parent,'');});

test('document folders stay independent across save/load and ignore legacy global shape',()=>{
 const docs=readDocumentFolders({'A.md':fixture(),'B.md':fixture()});
 moveItems(docs['A.md'],['f:one.md'],'c');
 assert.equal(docs['B.md'].positions['one.md'],'a');
 const saved=readDocumentFolders(JSON.parse(JSON.stringify(docs)));
 assert.equal(saved['A.md'].positions['one.md'],'c');
 assert.equal(saved['B.md'].positions['one.md'],'a');
