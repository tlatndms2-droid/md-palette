...
- 사용자가 `0단계 진행`을 승인하여 기본 플러그인, 빌드 구성, 기술 시험 및 0.0.1 배포를 수행했다.
- TypeScript·빌드·패키지 검사 2개, Sandbox 실제 설정 토글, 그룹 이동·Canvas 드롭 좌표 시험, 프로세스 재시작 검증을 통과했다.
- 공개 Release: https://github.com/tlatndms2-droid/md-palette/releases/tag/0.0.1
- BRAT 2.2.0을 통해 별도 Sandbox에 새로 설치·활성화하고 재시작 후에도 0.0.1을 확인했다.
- 현재 상태는 **0단계 사용자 BRAT 확인 대기**다. 사용자 본인의 확인은 아직 받지 않았으므로 전체 단계 완료로 처리하지 않는다. 1~7단계는 미시작이다.
- 0.0.1에는 Space·사이드바 등 제품 기능이 없다. 시험 도구는 `scripts/`에만 있고 배포된 `main.js`에는 포함되지 않는다.
...
사용자에게 BRAT으로 0.0.1 설치·활성화 및 재시작 상태를 확인받는다. 다음 단계 진행 의사가 확인되기 전에는 1단계를 구현하지 않는다.

1단계는 Main 지정, Sub·Reference 열기, 활성 파일 교환, 새 Main 지정·종료·순서 보호·재시작 복원 및 사이드바 기반이다. 해당 단계 원본 UI 이미지를 다시 대조하고 기능 충돌이 있는 영역은 사용자에게 확인한다.
...
- 같은 부모 아래 탭 그룹 재배치 및 Canvas 드롭 좌표는 Obsidian 1.13.7에서 시험했다. 내부 API를 사용하므로 중첩 분할·창 간 이동과 정식 기능 전체는 후속 단계에서 검증한다.
