...
기본·작업 브랜치: `codex/planning`. 공개 버전: **0.0.15**, 로컬 통합 후보: **0.1.0**.

## 현재 상태

**0.0.15 사용자 확인 및 7단계 진행 승인 완료. 0.1.0 로컬 통합·업데이트·재시작 검증 통과. GitHub 인증이 없어 push·Release·공개 자산 확인 미수행이며 7단계 전체 완료는 아니다.**

- 최신 실행 결과는 [7단계 검증 보고서](STAGE7_VALIDATION.md)를 따른다. 기능 소스 변경 없이 최종 버전과 검증 도구를 준비했다.
- 다음 작업은 GitHub 인증 → 커밋/push → 0.1.0 Release와 자산 해시 확인 → 사용자 BRAT 확인이다. 통과한 검증 빌드가 바뀌면 관련 재검증이 필요하다.
...
| 7 최종 통합 | 로컬 후보 0.1.0 | 로컬 검증 통과, push·Release·사용자 BRAT 확인 미수행, STAGE7_VALIDATION.md |

각 버전의 자동·Sandbox 검증은 사용자 직접 확인과 별개다. 0.0.12/13의 개별 사용자 확인 문장을 추정해서 추가하지 않는다. 코드가 배포되어 있다는 사실만으로 최종 통합 완료를 선언하지 않는다.

## 다음 작업

1. 이 PC의 GitHub 인증을 연결한다. 개인 인증 저장소의 비밀값을 추출하지 않는다.
2. 현재 검증 결과와 자산 해시를 유지한 채 커밋/push하고 새 0.1.0 Release를 만든다. 기존 공개 버전은 덮어쓰지 않는다.
3. 공개 자산 세 개를 다시 내려받아 해시를 확인한 뒤 사용자용 확인 항목을 제공한다.
4. 사용자의 0.1.0 BRAT 업데이트·정상 작동 확인을 기다린다.
